...
<body>
	<!-- 暂存ftid ready后jquery改变ftid选项 -->
	<input type="hidden" value="${sessionScope.edit_food.foodtype.ftid }"
		id="origin_ftid" />
	<form action="FoodUpdateServlet" method="post"
		onsubmit="return checkform();">
		<input type="hidden" name="fid" value="${sessionScope.edit_food.fid }" />
		<table class="table table-bordered table-hover definewidth m10">
			...
			<tr>
				...
				<td><input type="text" name="fname"
					value="${sessionScope.edit_food.fname }" /></td>
			</tr>
			<tr>
				<td class="tableleft">菜品描述</td>
				<td><textarea name="fdetial" rows=5 />${sessionScope.edit_food.fdetial }</textarea></td>
			</tr>
			<tr>
				<td class="tableleft">菜品单价</td>
				<td><input type="text" name="fprice"
					value="${sessionScope.edit_food.fprice }" /></td>
			</tr>
			<tr>
				<td class="tableleft">菜品图片</td>
				<td><img width="206"
					src="front-end/${sessionScope.edit_food.picture.ppath }"
					alt="${sessionScope.edit_food.picture.pid }" id="pic_show"> <span><a
						id="show_pic_lib" style="cursor: hand;">选取图片&gt;&gt;</a> </span> <input
					type="hidden" name="pid"
					value="${sessionScope.edit_food.picture.pid }" /></td>
			</tr>
			<tr>
				<td class="tableleft"></td>
				<td>
					<button type="submit" class="btn btn-primary" type="button">更新菜品</button>&nbsp;&nbsp;
...
<script type="text/javascript">
	$(function() {
		$('select[name="ftid"]').val($('#origin_ftid').val());
	});
</script>
